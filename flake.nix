{
  inputs = {
    node-measured.url = "github:input-output-hk/cardano-node/1.34.1";
    node-measured.inputs.nixpkgs.follows = "node-measured/haskellNix/nixpkgs-2105"; ## WARNING:  update this to match the measured node

    node-snapshot.url = "github:input-output-hk/cardano-node/1.34.1";
    node-snapshot.inputs.nixpkgs.follows = "node-measured/haskellNix/nixpkgs-2105";
    node-workbench.url = "github:input-output-hk/cardano-node/1.34.1";
    node-workbench.inputs.nixpkgs.follows = "node-measured/haskellNix/nixpkgs-2105";
  ...
  outputs = { ouroboros-network, self, nixpkgs, node-snapshot, node-measured, node-workbench, cardano-mainnet-mirror }@inputs: let
